...
class BertOutput(nn.Cell):
    ...
    def __init__(self,
        ...
        self.add = P.Add()
        self.layernorm = nn.LayerNorm((out_channels,), epsilon=layer_norm_eps).to_float(compute_type)
        self.cast = P.Cast()
...
class BertModel(nn.Cell):
    ...
    def construct(self, input_ids, token_type_ids=None, input_mask=None):
        """Bidirectional Encoder Representations from Transformers."""
        # embedding
        
        batch_size, seq_length = input_ids.shape
        if input_mask is None:
            input_mask = ms.ops.ones((batch_size, seq_length), ms.int64)
        
        if token_type_ids is None:
            if hasattr(self.bert_embedding_postprocessor, "token_type_ids"):
                buffered_token_type_ids = self.bert_embedding_postprocessor.token_type_ids[:, :seq_length]
                buffered_token_type_ids_expanded = buffered_token_type_ids.broadcast_to((batch_size, seq_length))
                token_type_ids = buffered_token_type_ids_expanded
            else:
                token_type_ids = ms.ops.zeros(input_ids.shape, ms.int64)
...
class BertEmbedding(nn.Cell):
    ...
    @classmethod
    def from_pretrained(self, config, is_training, ckpt_file):
        """
        Load the model parameters from checkpoint
        """
        bert_model = BertEmbedding(bert_config=config, is_training=is_training)
        param_dict = load_checkpoint(ckpt_file)
        
        load_param_into_net(bert_model, param_dict)
        return bert_model
    
    def construct(self, input_ids: Tensor, token_type_ids: Tensor = None, input_mask: Tensor = None) -> Tuple[
        ...
        """
        sequence_output, pooled_output, embedding_tables = self.bert(input_ids, token_type_ids, input_mask)
        return sequence_output, pooled_output, embedding_tables
